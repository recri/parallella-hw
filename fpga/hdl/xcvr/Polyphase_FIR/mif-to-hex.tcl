...
foreach file [glob coef*.mif] {
    ...
    close $fp
    set hex [file rootname $file].dat
    set fp [open "$hex" w]
	...
	set width [expr {([mif-width $mif]+3)/4}]
	set data [format %0${width}x $data]
	set data [string range $data end-[incr width -1] end]
	puts $fp $data
    }
    close $fp
}
